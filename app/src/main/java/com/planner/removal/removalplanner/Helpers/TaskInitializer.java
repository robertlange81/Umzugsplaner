...
public class TaskInitializer {
    ...
    public static void InitTasks(Date removalDate, Activity mainActivity) {
        ...
        Task rentalContractOld = new Task("Alten Mietvertrag kündigen", "Kündigen Sie Ihr altes Mietverhältnis fristgerecht. Möglicherweise können Sie die Kündigungsfrist durch einen Nachmieter verkürzen.", removalDate != null ? Calendar.getInstance().getTime() : null, Priority.High, 0L, new TaskType(TaskTypeMain.Contracts.getValue() | TaskTypeMain.OldFlat.getValue()));
        ...
        Task postalAftermath = new Task("Nachsendeauftrag", "Richten Sie einen Nachsendeauftrag ein.", addMonthDaysToJavaUtilDate(removalDate, 0, 0), Priority.Normal, 0L, new TaskType(TaskTypeMain.Contracts.getValue() | TaskTypeMain.Movement.getValue()));
        postalAftermath.addLink("TODO!!", "");
        Task.addTask(postalAftermath);

        Task informContractors = new Task("Vertragspartner informieren", "Informieren Sie Banken, Versicherungen, Mobilfunkanbieter, Arbeitgeber und andere Vertragspartner über Ihre neue Adresse. Verlassen Sie sich nicht nur auf den Nachsendeauftrag", addMonthDaysToJavaUtilDate(removalDate, 0, -14), Priority.Normal, 0L, new TaskType(TaskTypeMain.Contracts));
        Task.addTask(informContractors);

        Task currentContract = new Task("Energieversorger", "Prüfen Sie, ob Sie Ihren alten Gas - bzw. Stromvertrag behalten (aber über Adresswechsel informieren) oder wechseln müssen / dürfen.", addMonthDaysToJavaUtilDate(removalDate, 0, -7), Priority.High, 0L, new TaskType(TaskTypeMain.Contracts));
        currentContract.addLink("Anbieter vergleichen", "https://www.verivox.de"); // FIXME: partner-id
        Task.addTask(currentContract);

        Task internetContract = new Task("Internetanschluss", "Prüfen Sie, ob Sie Ihren Anbieter wechseln müssen / dürfen. Andernfalls muss dieser zumindest informiert werden.", addMonthDaysToJavaUtilDate(removalDate, 0, -7), Priority.High, 0L, new TaskType(TaskTypeMain.Contracts));
        ...
        Task phoneContract = new Task("Festnetztelefon", "Prüfen Sie, ob Sie Ihren Anbieter wechseln müssen / dürfen und ob eine Mitnahme der alten Nummer möglich ist.", addMonthDaysToJavaUtilDate(removalDate, 0, -7), Priority.High, 0L, new TaskType(TaskTypeMain.Contracts));
        phoneContract.addLink("Telefonanbieter vergleichen", "https://www.verivox.de"); // FIXME: partner-id
        Task.addTask(phoneContract);

        Task tvContract = new Task("TV-Anschluss", "Prüfen Sie, ob Sie Ihren Anbieter wechseln müssen / dürfen. Andernfalls muss dieser zumindest informiert werden.", addMonthDaysToJavaUtilDate(removalDate, 0, -7), Priority.High, 0L, new TaskType(TaskTypeMain.Contracts));
        tvContract.addLink("TV-Anbieter vergleichen", "https://www.verivox.de"); // FIXME: partner-id
        Task.addTask(tvContract);

        Task fireAndBurglaryProtection = new Task("Brandschutz und Einbruchsschutz", "Rauchmelder sind in einigen Bundesländern Pflicht und können Leben retten. Vergessen Sie zudem nicht den Einbruchsschutz.", removalDate != null ? addMonthDaysToJavaUtilDate(removalDate, 0, -6) : null, Priority.Normal, 0L, new TaskType(TaskTypeMain.NewFlat));
        ...
        Task requestSpecialLeave = new Task("Sonderurlaub beantragen", "In der Regel genehmigt Ihnen Ihr Arbeitgeber für den Umzug Sonderurlaub. Fragen kostet nichts!", Calendar.getInstance().getTime(), Priority.Normal, 0L, new TaskType(TaskTypeMain.Movement));
